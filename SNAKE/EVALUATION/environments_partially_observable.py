...
class BaseEnvironment:
    ...
    def __init__(self, n_boards, board_size, mask_size):
        ...
        self.FRUIT_REWARD = 1.25        
        self.LENGTH_REWARD_FACTOR = 0.15 
        self.STEP_REWARD = -0.025       
        self.ATE_HIMSELF_REWARD = -5.0  
        self.HIT_WALL_REWARD = -5.0   
    ...
    def move(self, actions):
        ...
        lengths = np.array([len(b) + 1 for b in self.bodies])

        rewards[boards_where_fruits_is_been_eaten] = self.FRUIT_REWARD + (lengths[boards_where_fruits_is_been_eaten] * self.LENGTH_REWARD_FACTOR)

        dynamic_step_rewards = self.STEP_REWARD / (0.1 * lengths)
        
        step_mask = np.setdiff1d(
        ...
        rewards[step_mask] = dynamic_step_rewards[step_mask]
